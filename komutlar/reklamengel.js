...
exports.run = async(client, message, args) => {
  ...
  if (!message.member.hasPermission('MANAGE_GUILD')) return message.channel.send(` Bu komudu kullanabilmek için yetkiye sahip olman gerek.`)
  if (!args[0]) return message.channel.send(`:no_entry: Reklam Filtresini Ayarlamak İçin \`-jreklam aç\` | Kapatmak İstiyorsanız \`-jreklam kapat\` Yazabilirsiniz`)
  if (args[0] !== 'aç' && args[0] !== 'kapat') return message.channel.send(`:no_entry: Reklam Filtresini Ayarlamak İçin \`-jreklam aç\` | Kapatmak İstiyorsanız \`-jreklam kapat\` Yazabilirsiniz`)
